...
menu = st.sidebar.selectbox(
    ...
    ["🏠 Início", "📝 Registrar Empréstimo", "🔄 Registrar Devolução", "⏰ Empréstimos em Atraso"]
)
...
if menu == "🏠 Início":
    ...
    with col1:
        equip_id = st.number_input("ID do Equipamento", min_value=1, value=1, step=1)
        nome = st.text_input("Nome do Usuário")
    
    with col2:
        email = st.text_input("Email do Usuário")
        dias = st.number_input("Dias para Devolução", min_value=1, value=7)

    if st.button("✅ Registrar Empréstimo", type="primary"):
        sucesso = sistema.registrar(equip_id, nome, email, dias)
        if sucesso:
            st.success(f"Empréstimo registrado com sucesso para {nome}!")
        else:
            st.error("❌ Equipamento não encontrado ou indisponível.")

elif menu == "🔄 Registrar Devolução":
    st.subheader("Registrar Devolução")
    emprestimo_id = st.number_input("ID do Empréstimo", min_value=1, value=1)
    ...
    # Como o método atual usa print, vamos melhorar um pouco
    atrasados = sistema._servico.repositorio.listar_em_atraso() if hasattr(sistema, '_servico') else []
    
    if not atrasados:
        st.info("Nenhum empréstimo em atraso no momento.")
    else:
        for emp in atrasados:
            st.warning(f"ID {emp.id} - {emp.usuario_nome} - Devolução prevista: {emp.data_devolucao}")
